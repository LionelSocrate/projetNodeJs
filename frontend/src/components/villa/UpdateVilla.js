...
export default function UpdateVilla({
  ...
  const [typeUpdate, setTypeUpdate] = useState("Villa Basse");
  const [nomUpdate, setNomUpdate] = useState("");
  const [projetUpdate, setProjetUpdate] = useState("Andakana");
    ...
    try {
      await axios({
        ...
        data: {
          ...
          type: typeUpdate,
          lot: lotUpdate,
          idProjet: projetUpdate,
          description: descriptionUpdate,
          picture: `/uploads/villa/${nomUpdate + lotUpdate}.jpg`,
        },
      }).then((res) => {});
      const formData = new FormData();
      formData.append("nom", nomUpdate);
      formData.append("lot", lotUpdate);
      ...
      <div className=" fixed top-0 left-0 w-screen h-screen z-50 bg-[var(--bgFixed)] flex items-center justify-center ">
        ...
        >
          ...
          <div className=" w-full flex items-center gap-8">
            <div className="relative w-[18rem] h-[15rem]  ">
              <label
                htmlFor={"file"}
                className={
                  " relative h-full w-full  rounded-md flex justify-center items-center  cursor-pointer overflow-hidden"
                }
              >
                <img
                  src={imageSrc}
            ...
            <div className=" flex flex-col gap-4">
              ...
              <div className=" flex items-center gap-4">
                <SelectVilla
                  ...
                  value={projetUpdate}
                  onchange={(e) => {
                    setProjetUpdate(e.target.value);
                  }}
                />
                <SelectVilla
